                ...
                with st.spinner("Processing protein..."):
                    ...
                    if st.session_state.ptm_enabled and ptm_dict:
                        ...
                        if all_unimods:
                            ...
                            # Initialize custom labels in session state if not already set
                            if 'ptm_configs' not in st.session_state:
                                st.session_state.ptm_configs = {}
                            ...
                            for um in all_unimods:
                               ...
                               # Initialize config for this um if missing
                            ...
                            colA, colB, colC = st.columns([2, 1, 1, ])  # added wider column for text input

                            with colA:
                                # Show current (custom) label in display
                            ...
                            with colC:
                                ...
                                st.session_state.ptm_configs[um]['label'] = st.text_input(
                                    f"Custom name for {um}",
                                    value=st.session_state.ptm_configs[um]['label'],
                                    placeholder="e.g. Phosphorylation, Acetyl-K",
                                    key=f"ptmlabel_{um}"
                                )

                            # Use the UPDATED custom label here
                            display_label = st.session_state.ptm_configs[um]['label']

                            # Save to final_ptm using the custom label
                            final_ptm[um] = {
                                "positions": positions,
                                "color": st.session_state.ptm_configs[um]['color'],
                                "label": display_label,              # ← now uses custom name!
    ...
    with quant_multi_tab:
            ...
            with st.expander("Aggregation & Options", expanded=st.session_state.step1_done and not st.session_state.step2_done):
                if not st.session_state.step1_done:
                    ...
                    if st.session_state.all_unimods:
                        ...
                        unimod_links = []
                        for um in st.session_state.all_unimods:
                            m = re.search(r"(\d+)", str(um))
                            uid = m.group(1) if m else str(um)
                            url = f"https://www.unimod.org/modifications_view.php?editid1={uid}"
                            link = f'<a href="{url}" target="_blank" style="color:#87CEEB; text-decoration:underline;">UniMod:{uid}</a>'
                            unimod_links.append(link)
                    
                        if unimod_links:
                            st.markdown(
                                "Detected UniMod IDs (click to view details): " + ", ".join(unimod_links),
                                unsafe_allow_html=True,
                            )
                        else:
                            st.write("No UniMod IDs detected.")
                    
                        st.write("Select the ones to include in the analysis:")
                    
                        # ✅ FIX: initialize ONLY if the key is absent — never overwrite on rerun
                        if "selected_unimods" not in st.session_state:
                            st.session_state.selected_unimods = st.session_state.all_unimods.copy()
                    
                        # Let the widget manage its own state via its key; read back below
                        st.multiselect(
                            "Select UniMod IDs to Include",
                            options=st.session_state.all_unimods,
                            default=st.session_state.selected_unimods,   # only used on first render
                            help="Choose the UniMod IDs you want to process for PTM annotation.",
                            key="selected_unimods_widget",               # ← unique stable key
                        )
                        # Sync the widget value back to the canonical session-state key
                        # (the widget stores its current value under its own key)
                        st.session_state.selected_unimods = st.session_state.selected_unimods_widget
            ...
            with st.expander(" Select Protein & Process", 
                    ...
                    if not st.session_state.step3_done:
                        ...
                        with col3:
                            combine_isoforms = st.selectbox("Combine Isoforms?", ["yes", "no"],key="combine_iso")
                        with col4:
                            overlap_strategy = st.selectbox("Overlap Strategy", ["none","merge", "highest"],key="overlap_strat")
                    ...
                    if (
                        ...
                        st.caption(
                            "Changes here update the visualizations immediately — "
                        ...
                        for um in st.session_state.selected_unimods:
                            ...
                            unimod_id = m_id.group(1) if m_id else str(um)
                            ...
                            with col_ptm1:
                                ...
                                chk_key = f"ptm_cfg_chk_{um}"
                                st.checkbox(
                                    f"Include UniMod:{unimod_id}",
                                    value=st.session_state.ptm_configs[um]["selected"],
                                    key=chk_key,
                                )
                                st.session_state.ptm_configs[um]["selected"] = st.session_state[chk_key]

                            with col_ptm2:
                                lbl_key = f"ptm_cfg_lbl_{um}"
                                st.text_input(
                                    f"Display label for UniMod:{unimod_id}",
                                    value=st.session_state.ptm_configs[um]["label"],
                                    key=lbl_key,
                                )
                                st.session_state.ptm_configs[um]["label"] = st.session_state[lbl_key]

                            with col_ptm3:
                                clr_key = f"ptm_cfg_clr_{um}"
                                st.color_picker(
                                    f"Color for UniMod:{unimod_id}",
                                    value=st.session_state.ptm_configs[um]["color"],
                                    key=clr_key,
                                )
                                st.session_state.ptm_configs[um]["color"] = st.session_state[clr_key]

                            st.caption(
                                f"Will display as: **{st.session_state.ptm_configs[um]['label']}**"
                            )
                    ...
                    if st.session_state.get("step4_done", False):
                        ...
                        # Use current session value as default, fallback to initial defaults
                        bg_key   = "appearance_bg"
                        cmap_key = "appearance_cmap"
                        nmc_key  = "appearance_nmc"

                        st.selectbox(
                            "Background Color",
                            ["black", "white", "darkgrey"],
                            index=["black", "white", "darkgrey"].index(
                                st.session_state.get("bg_color", "black")
                            ),
                            key=bg_key,
                        )
                        st.session_state.bg_color = st.session_state[bg_key]
                        ...
                        st.selectbox(
                            ...
                            index=cmap_options.index(
                                st.session_state.get("selected_cmap", "autumn")
                            ),
                            key=cmap_key,
                        )
                        st.session_state.selected_cmap = st.session_state[cmap_key]

                        st.color_picker(
                            "Select Not Mapped Color",
                            value=st.session_state.get("not_mapped_color", "#d3d3d3"),
                            key=nmc_key,
                        )
...
with quant_diff_tab:
        ...
        with st.expander("Analysis Options", expanded=st.session_state.diff_step1_done and not st.session_state.diff_step2_done):
            if not st.session_state.diff_step1_done:
                ...
                st.session_state.ptm_enabled = st.checkbox(
                    ...
                    key="quant_diff_ptm_enable"
                )
                st.session_state.apply_tryptic = st.checkbox(
                    "Apply Tryptic Rule (K/R cleavage)",
                    value=st.session_state.get("apply_tryptic", False)
                )
                st.session_state.proteotypic_only = st.checkbox(
                ...
                if st.session_state.ptm_enabled and st.session_state.diff_all_unimods:
                    ...
                    # Clickable links
                    unimod_links = []
                    for um in st.session_state.diff_all_unimods:
                        m = re.search(r"(\d+)", str(um))
                        uid = m.group(1) if m else str(um)
                        url = f"https://www.unimod.org/modifications_view.php?editid1={uid}"
                        link = f'<a href="{url}" target="_blank" style="color:#87CEEB; text-decoration:underline;">UniMod:{uid}</a>'
                    ...
                    # initialize ONLY when absent — never overwrite on rerun
                    if "diff_selected_unimods" not in st.session_state:
                        st.session_state.diff_selected_unimods = []   # empty by default — user must choose

                    # Use stable widget key; sync back after render
                    st.multiselect(
                        "Select UniMod IDs to Include",
                        options=st.session_state.diff_all_unimods,
                        default=st.session_state.diff_selected_unimods,
                        help="Select only the modifications you want to visualize.",
                        key="diff_unimod_multiselect",
                    )
                    # Sync widget value → canonical session state key
                    st.session_state.diff_selected_unimods = st.session_state.diff_unimod_multiselect

                    if not st.session_state.diff_selected_unimods:
                        st.warning("No UniMod IDs selected — PTM annotations will not appear even though PTM is enabled.")
        ...
        with st.expander("🔬 Select Protein & Visualization Settings",expanded=st.session_state.diff_step2_done and not st.session_state.diff_step3_done):
            if not st.session_state.diff_step2_done:
                ...
                apply_log2 = st.session_state.get("apply_log2", False)
                apply_neglog10 = st.session_state.get("apply_neglog10", False)
                ...
                with col1:
                    ...
                    if "diff_combine_isoforms" not in st.session_state:
                        st.session_state.diff_combine_isoforms = "yes"
                    combine_isoforms = st.selectbox(
                        "Combine Isoforms?", 
                        ["yes", "no"], 
                        key="diff_combine_isoforms",
                        index=0 if st.session_state.diff_combine_isoforms == "yes" else 1
                    )
                with col2:
                    # Initialize in session_state if not exists
                    if "diff_overlap_strategy" not in st.session_state:
                        st.session_state.diff_overlap_strategy = "none"

                    overlap_strategy = st.selectbox(
                        "Overlap Strategy", 
                        ["none", "merge", "highest"], 
                        key="diff_overlap_strategy",
                        index=["none", "merge", "highest"].index(st.session_state.diff_overlap_strategy)
                    )
                ...
                if st.button("Process Protein", use_container_width=True,key="process_protein_diff"):
                    ...
                    st.session_state.diff_apply_log2          = st.session_state.apply_log2
                    st.session_state.diff_apply_neglog10      = st.session_state.apply_neglog10
        ...
        with st.expander("🧬 Processing Results",expanded=st.session_state.get("diff_step3_done", False) and not st.session_state.get("diff_processed", False)):
            if not st.session_state.diff_step3_done:
                ...
                for condition in conditions:
                    ...
                    if st.session_state.ptm_enabled:
                        for um, positions in ptms.items():
                            if um in st.session_state.diff_selected_unimods:
                                config = st.session_state.diff_ptm_configs.get(um, {})
                                ptm_data[condition][um] = {
                                    'positions': sorted(list(positions)),
                                    'selected': config.get('selected', True),
                                    'color': st.session_state.diff_ptm_configs.get(um,{}).get('color', '#3700FF'),
                                    'label': config.get('label', um)
                                }
            ...
            if (
                ...
                st.subheader("PTM Configuration")
                st.caption("Changes apply immediately — no reprocessing needed.")
                ...
                for um in st.session_state.diff_selected_unimods:
                    ...
                    with c1:
                        st.markdown(
                            f'<a href="{direct_url}" target="_blank" style="color:#2b8cff; '
                            f'text-decoration:underline; font-weight:bold;">UniMod:{unimod_id}</a>',
                            unsafe_allow_html=True,
                        )
                        chk_key = f"diff_ptm_cfg_chk_{um}"
                        st.checkbox(f"Include UniMod:{unimod_id}",
                                    value=st.session_state.diff_ptm_configs[um]["selected"],
                                    key=chk_key)
                        st.session_state.diff_ptm_configs[um]["selected"] = st.session_state[chk_key]
                    with c2:
                        lbl_key = f"diff_ptm_cfg_lbl_{um}"
                        st.text_input(f"Display label for UniMod:{unimod_id}",
                                      value=st.session_state.diff_ptm_configs[um]["label"],
                                      key=lbl_key)
                        st.session_state.diff_ptm_configs[um]["label"] = st.session_state[lbl_key]
                    with c3:
                        clr_key = f"diff_ptm_cfg_clr_{um}"
                        st.color_picker(f"Color for UniMod:{unimod_id}",
                                        value=st.session_state.diff_ptm_configs[um]["color"],
                                        key=clr_key)
                        st.session_state.diff_ptm_configs[um]["color"] = st.session_state[clr_key]

                    st.caption(f"Will display as: **{st.session_state.diff_ptm_configs[um]['label']}**")
                ...
                for cond in st.session_state.get("diff_conditions", []):
                    for um, entry in st.session_state.diff_ptm_data.get(cond, {}).items():
                        if um in st.session_state.diff_ptm_configs:
                            cfg = st.session_state.diff_ptm_configs[um]
                            entry["color"]    = cfg["color"]
                            entry["label"]    = cfg["label"]
                            entry["selected"] = cfg["selected"]
        ...
        with st.expander("🧪 3D Sturctural Visualizations", expanded=st.session_state.diff_processed):
            if not st.session_state.diff_processed:
                ...
                if show_full:
                    ...
                    # Create a temporary normalized column 
                ...
                if view_mode == "Full Structure (All Peptides)" or selected_peptide is None:
                    ...
                    for cond in conditions:
                        ...
                        if st.session_state.ptm_enabled and temp_ptms:
                            for um, positions in temp_ptms.items():
                                if um in st.session_state.diff_selected_unimods:
                                    cfg = st.session_state.diff_ptm_configs.get(um, {})
                                    cond_ptm[um] = {
                                        "positions": sorted(list(positions)),
        ...
        with st.expander("📈 Peptide Analysis Plots", expanded=True):            
                    ...
                    if 'protein_seq' not in st.session_state or not st.session_state.protein_seq:
                        ...
                        for _, r in plot_df.iterrows():
                            rgb     = tuple(int(r['color'].lstrip('#')[i:i+2], 16) for i in (0, 2, 4))
                            opacity = (
                            ...
                            fig.add_trace(go.Scatter(
                                ...
                                fill='toself', fillcolor=f"rgba{rgb + (opacity,)}",
